...
import { copyFromSearchParams, getElement, normalizeElements, type SearchWithElement } from './getElement'
...
export type ElementType = 'input' | 'select' | 'rangePicker' | 'datePicker' | 'textarea' | 'rangeInput' | 'custom'
...
export interface FormConfig {
	onValuesChange?(changed: Record<string, any>, changedValues: Record<string, any>, form: FormInstance): void
}
...
const SearchForm = forwardRef<any, SearchFormProps<any>>((props, ref) => {
	...
	const { formConfig, enableFormCollapse, hiddenFormButtons, ...globalConfig } = useTablePageConfig()
	const [form] = Form.useForm()
	const wrapperRef = useRef<HTMLDivElement>(null)
	const [searchParams] = useSearchParams()

	const { normalizedItems, queryKeyByFormKey, formToQueryObject, queryToFormObject, defaultValues } = useMemo(
		() => normalizeElements(searchs, globalConfig),
		[searchs, globalConfig]
	)
	...
	const initialValues = getInitialValues(searchParams, queryToFormObject, queryKeyByFormKey, defaultValues)
